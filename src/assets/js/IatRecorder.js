...
const IatRecorder = class {
    ...
    let getMediaSuccess = (stream) => {
      ...
      this.scriptProcessor.onaudioprocess = (e) => {
        ...
        if (this.status === "ing") {
          ...
          this.transWorker.postMessage(audioData);  // 使用 this.transWorker
          // 如果 onAudioProcess 回调被设置了，就调用它，并将音频数据作为参数传递
          this.onAudioProcess && this.onAudioProcess(audioData);
        }
      };
  ...
  recorderStop() {
    ...
    // 停止音频流
    if (this.mediaSource && this.mediaSource.mediaStream) {
      this.mediaSource.mediaStream.getTracks().forEach((track) => {
        track.stop();
      });
    ...
    if (this.audioContext) {
      ...
      this.audioContext = null; // 清除音频上下文，以便下次重新创建
    }
    this.setStatus("end");
    // 如果有必要，通知外部状态已变更
    this.onStatusChange && this.onStatusChange("end");
  }
